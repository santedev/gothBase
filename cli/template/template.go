...
func GenerateTemplate(selections Selections) error {
	...
	if (selections.Tailwind || selections.Default) && !selections.Minimalistic {
		if err := runCommand(
			...
			return fmt.Errorf("failed to download TailwindCSS npm package: %w", err)
		}
		if err := os.MkdirAll(path.Join(projectDir, "public/styles"), os.ModePerm); err != nil {
			return fmt.Errorf("failed to create directory %s: %w", pubDir, err)
		}
		if err := runCommand(
			projectDir,
			"npx", "tailwindcss", "-i", "tailwind/css/app.css", "-o", "public/styles/styles.css"); err != nil {
			return fmt.Errorf("failed to generate css file using tailwindcss: %w", err)
		}
	}
	if (selections.Esbuild || selections.Default) && !selections.Minimalistic {
		if err := runCommand(
			projectDir,
			"npm",
			"install",
			...
			return fmt.Errorf("failed to download Esbuild npm package: %w", err)
		}
		if err := os.MkdirAll(path.Join(projectDir, "public/styles"), os.ModePerm); err != nil {
			return fmt.Errorf("failed to create directory %s: %w", pubDir, err)
		}
		if err := os.MkdirAll(path.Join(projectDir, "public/modules"), os.ModePerm); err != nil {
			return fmt.Errorf("failed to create directory %s: %w", pubDir, err)
		}
		f, err := os.Create(path.Join(projectDir, "public/modules/main.js"))
		if err != nil {
			return err
		}
		f.Close()
		f, err = os.Create(path.Join(projectDir, "public/styles/main.css"))
		if err != nil {
			return err
		}
		f.Close()
