...
func stepView3(m model) string {
	...
	s += boldGrayStyle.Render(fmt.Sprintf(" %d/%d", m.steps, m.limit))
	if m.done {
		s += "\n\n" + boldStyle.Render("Run the following commands after generating the project:")
		s += "\n `templ generate`\n"
		s += " `air`"
	}
...
func initialModel() model {
	...
	return model{
		choices: [][]string{
			{},
			{"default", "minimalistic", "auth", "database sql", "Dockerfile", "javascript", "htmx", "tailwind", "esbuild"},
			{},
		},
		limit:     3,
		steps:     1,
